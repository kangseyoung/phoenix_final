...
class PublishHandler(QMainWindow,DataExplorer):
    ...
    def first_show(self): #self.path 설정 #마야이면, 마야 경로가, 누크면 누크 경로가 세팅된다.
        self.ui.label_username.setText(self.get_user_name())
        self.ui.label_project_name.setText(self.get_project_name())
    ...
    def load_file(self, *file_paths): #파일 이름을 ui에 개시
        ...
        for file_path in file_paths:
            if os.path.exists(file_path) and file_path.endswith(('.mb', '.ma', '.nknc', '.nk')):
                ...
                if i:
                    self.add_outliner_items(tree_item,self.get_outliner_items(),"Outliner") ################수정
    ...
    def get_file_info(self, item, column): #파일의 type을 ui에 띄운다
        ...
        if file_extension in ['.mb', '.ma']:
            pixmap = QPixmap("/home/rapa/_phoenix_/ui/logo/maya.png")
            file_type = "Maya Session"
        elif file_extension in ['.nk', '.nknc']:
            pixmap = QPixmap("/home/rapa/_phoenix_/ui/logo/Nuke.png")
    ...
    def check_existence_of_version(self):
        ...
        print(file)
        version = self.sg.find_one('Version', [['code', 'is', file]], ['sg_task','entity']) #현재 파일의 version을 찾음 없으면, 메시지 띄우기,z
        print(version)
    ...
    def save_checked_data(self): #선택된 모델들의 텍스트를 활용해 펍합니다.
        ...
        if self.task_step == "lkd":
            ...
            print("saving lgt...")
            aovs = self.get_standard_aov_info(254)################################################수정해야함
            print("aovs",aovs)
