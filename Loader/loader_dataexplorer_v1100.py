...
class ThumbnailManager:
    def __init__(self, sg, save_dir: str = "project_thumbnails"):
        self.sg = sg
    ...
    def download_version_thumbnail(self, version_id: int, version_type: str) -> Optional[str]:
        ...
        base_path = f'{os.path.dirname(os.path.abspath(__file__))}/version_thumbnail'   # 썸네일 저장 디렉토리
...
class DataExplorer:
    ...
    def __init__(self, sg, email):
        ...
        self.save_dir = self.config.get_value_as_str('Paths', 'thumbnail_dir')
        os.makedirs(self.save_dir, exist_ok=True)
        # 썸네일 관리자 초기화
        self.thumbnail_manager = ThumbnailManager(self.sg, self.save_dir)
        # 현재 사용자 정보 가져오기
        self.email= email
    ...
    def get_version_metadata(self, version_code: str, version_type: str = None) -> Dict[str, Any]:  # 버전 메타데이터 가져오기
        ...
        fields = ['code', 'sg_status_list', 'created_at', 'created_by', 'description',  
                'sg_path_to_frames', 'sg_first_frame', 'sg_last_frame', 'sg_task', 'entity','sg_path', 'sg_version_file_type', 'sg_path']    
    ...
    def get_version_pub_metadata(self, version_code: str, version_type: str = None) -> Dict[str, Any]:  # 버전 메타데이터 가져오기
        print(f"Attempting to retrieve metadata for version: {version_code}, type: {version_type}") # 디버그 로그 추가
    ...
    def get_pub_version_data_from_published_files(self, version_id: int) -> Dict[str, Any]:  # pub 상태인 버전의 shotgun puublished files의 데이터 가져오기
        ...
        publish = self.sg.find_one("PublishedFile", 
                                   ...
                                   ["code", "published_file_type", "version_number", "path", "created_at", "created_by", "description"])
    ...
    def get_projects(self) -> Tuple[Dict[str, Any]]:
        ...
        """
        projects = self.sg.find("Project", [], ["name", "id", "sg_status", "image"])
    ...
    def get_publishedfile_details(self, entity_type: str, entity_id: int) -> Tuple[Dict[str, Any]]:
        ...
        publishes = self.sg.find("PublishedFile",
                                 ...
                                 ['code', 'id', 'published_file_type', 'version_number', 'created_at', 'created_by', 'description'])
    ...
    def get_shot_versions(self, shot_id: int) -> List[Dict[str, Any]]:
        ...
        versions = self.sg.find("Version",
                                ...
                                ["code", "description", "sg_status_list", "created_at",
                                  "created_by", "id"])
    ...
    def get_version(self, task_id: int) -> List[Dict[str, Any]]:
        ...
        versions = self.sg.find("Version", 
                                ...
                                ["code", "sg_status_list", "id", "created_at", "user", "description"])
...
class DataManagerForSaver:
    ...
    _lock = Lock()    # 스레드 안전성을 위한 락
    ROOT_PATH = '/home/rapa/phoenix_pipeline_folders/pipeline/launcher/loader/data_from_loader'  # 캐시 디렉토리의 루트 경로

    def __new__(cls):
        """
        싱글톤 패턴을 구현하는 메서드입니다.
        이 클래스의 인스턴스가 하나만 생성되도록 보장합니다.
        """
        with cls._lock:  # 스레드 안전성을 위해 락을 사용
...
# 사용 예시
# data_manager = DataManagerForSaver()
# data_manager.save_data_for_saver({"key": "value"})
# loaded_data = data_manager.load_data('json_from_loader.json')
# if loaded_data:
#     print(loaded_data)
# else:
